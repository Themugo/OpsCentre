...
const FROM_EMAIL     = process.env.RESEND_FROM_EMAIL ?? "noreply@opscentre.io";
const FROM_NAME      = process.env.RESEND_FROM_NAME  ?? "OpsCentre";
...
function smsTemplate(event: NotifEvent, p: NotifPayload): string {
  ...
  switch (event) {
    case "invoice_created":
      return `OpsCentre: Hi ${p.customerName}, your invoice ${p.invoiceNo} for ${kes(p.amountKes!)} is ready. Please pay to keep your connection active. Reply STOP to opt out.`;
    case "invoice_overdue":
      return `OpsCentre: REMINDER - Invoice ${p.invoiceNo} (${kes(p.amountKes!)}) is overdue. Pay now to avoid suspension. M-Pesa: ${process.env.MPESA_SHORTCODE ?? "123456"}. Acc: ${p.invoiceNo}.`;
    case "payment_received":
      return `OpsCentre: Payment confirmed! ${kes(p.amountKes!)} received. Receipt: ${p.receiptNo}. Thank you, ${p.customerName}.`;
    case "subscription_suspended":
      return `OpsCentre: Your ${p.planName} connection has been suspended due to non-payment. Pay ${p.invoiceNo} to reconnect. Call us: ${process.env.SUPPORT_PHONE ?? "0800 000 000"}.`;
    case "subscription_activated":
      return `OpsCentre: Welcome ${p.customerName}! Your ${p.planName} connection is now active. Enjoy fast fiber internet!`;
    case "plan_upgraded":
      return `OpsCentre: Your plan has been upgraded to ${p.planName}. Your new speeds are now active. Enjoy!`;
    case "ticket_opened":
      return `OpsCentre: Support ticket ${p.ticketNo} opened. Our team will respond within 24 hours. Track at portal.opscentre.io`;
    case "ticket_resolved":
      return `OpsCentre: Your ticket ${p.ticketNo} has been resolved. We hope your issue is fixed! Rate us at portal.opscentre.io`;
    case "field_job_scheduled":
      return `OpsCentre: A technician is scheduled to visit on ${p.jobDate}. Please ensure access to your premises. Call us if you need to reschedule.`;
    case "outage_alert":
      return `OpsCentre: We're aware of a network issue affecting ${p.nodeNames?.join(", ")}. Our team is working on a fix. We apologise for the inconvenience.`;
    case "welcome":
      return `OpsCentre: Welcome to OpsCentre, ${p.customerName}! Your account is active. Visit portal.opscentre.io to manage your connection.`;
    default:
      return `OpsCentre notification for ${p.customerName}.`;
  }
}
  ...
  const base = (title: string, body: string) => `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:24px;">
      <div style="background:#1D9E75;border-radius:8px 8px 0 0;padding:20px 24px;">
        <span style="color:#fff;font-size:18px;font-weight:500;">OpsCentre</span>
      </div>
      <div style="background:#fff;border:1px solid #e5e7eb;border-radius:0 0 8px 8px;padding:28px 24px;">
        ...
        <p style="font-size:12px;color:#9ca3af;margin:0;">
          OpsCentre · Nairobi, Kenya · <a href="https://portal.opscentre.io">Customer portal</a>
        </p>
      </div>
    </div>`;
    ...
    case "invoice_created":
      return {
        ...
        html: base("New invoice", `
          ...
          <a href="https://portal.opscentre.io/invoices" style="display:inline-block;background:#1D9E75;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:500;">Pay now</a>
        `),
      };
    case "payment_received":
      return {
        ...
        html: base("Payment confirmed", `
          ...
          <a href="https://portal.opscentre.io/invoices" style="display:inline-block;background:#1D9E75;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:500;">View invoice</a>
        `),
      };
    default:
      return {
        subject: `OpsCentre notification`,
        html: base("OpsCentre", `<p>Hi ${p.customerName}, you have a notification from OpsCentre.</p>`),
      };
  }
}
